...
import { Ad, MagazineIssue, NewsPost } from '../../../vartmaan-shared-types.js';
...
export class ContentService {
  ...
  async createMagazine(magazineData: Omit<MagazineIssue, 'id' | 'createdAt' | 'updatedAt'>) {
    const fallbackPages =
      Array.isArray(magazineData.pages) && magazineData.pages.length > 0
        ? magazineData.pages
        : (magazineData.coverImage ? [magazineData.coverImage] : []);
  ...
  async reworkMagazine(id: string, user: UserRecord, reason?: string) {
    const magazine = await this.updateMagazine(id, {
      status: 'DRAFT',
      rejectionReason: reason || 'Rework requested'
    } as any);
    ...
    await store.recordAudit({
      ...
      details: { reason, note: 'Sent for rework' }
    });
    return magazine;
  }
