...
function initialArticleStatus(role: UserRole | undefined): string {
  if (role === UserRole.EDITOR || role === UserRole.ADMIN) return 'DRAFT';
  return 'APPROVED';
}

function initialMagazineStatus(role: UserRole | undefined): string {
  if (role === UserRole.EDITOR || role === UserRole.ADMIN) return 'DRAFT';
  return 'APPROVED';
}
...
export const createArticle = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const article = await contentService.createArticle({
    ...
    status: initialArticleStatus(req.user?.role),
...
export const createMagazine = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const magazine = await contentService.createMagazine({
    ...req.body,
    status: initialMagazineStatus(req.user?.role),
    submittedBy: req.user?.id
  });
...
export const reworkMagazine = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const magazine = await contentService.reworkMagazine(req.params.id as string, req.user!, req.body?.reason);
  res.json({ success: true, message: 'Magazine sent back for rework.', data: { magazine } });
});
