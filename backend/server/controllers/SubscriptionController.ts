...
import { MAX_SUBSCRIPTION_SCREENSHOT_BYTES } from '../utils/subscriptionInput.js';
...
export const approveSubscriptionRequest = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const request = await subscriptionService.approveAndActivateSubscriber(req.params.id as string, req.user);
...
export const rejectSubscriptionRequest = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const request = await subscriptionService.updateSubscriptionRequest(req.params.id as string, 'REJECTED', req.user, reason);
...
/** POST /api/subscribers/digital — JSON + reCAPTCHA; activates digital access immediately */
export const registerDigitalSubscriber = asyncHandler(async (req: Request, res: Response) => {
  ...
  const request = await subscriptionService.registerDigitalFromPopup({
  ...
  res.status(201).json({
    success: true,
    message: 'Digital access activated.',
    data: { subscriberId: request.id }
  });
});

/** POST /api/subscribers/physical — multipart + reCAPTCHA; pending admin approval */
export const registerPhysicalSubscriber = asyncHandler(async (req: Request, res: Response) => {
  const file = req.file;
  if (!file) {
    throw new AppError('Payment screenshot is required.', 400);
  }
  if (file.size > MAX_SUBSCRIPTION_SCREENSHOT_BYTES) {
    throw new AppError('Screenshot must be 5MB or smaller.', 400);
  }
  ...
  const request = await subscriptionService.registerPhysicalFromPopup({
    name: String(body.name ?? ''),
    email: String(body.email ?? ''),
    phone: String(phone ?? ''),
