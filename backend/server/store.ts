...
const INITIAL_NEWS: NewsPost[] = ARTICLE_CATEGORIES.map((category, index) => ({
  id: `seed-article-${index + 1}`,
  title: `${category}: Key Developments This Week`,
  category,
  excerpt: `A concise roundup of the most important ${category.toLowerCase()} developments and why they matter.`,
  content: `This report summarizes recent developments in ${category.toLowerCase()} with a focus on public impact, emerging trends, and expert commentary. The editorial desk will continue tracking this story as new information becomes available.`,
  image: `https://images.unsplash.com/photo-${index % 2 === 0 ? '1495020689067-958852a7765e' : '1504711434969-e33886168d8c'}?w=800&h=500&fit=crop`,
...
export class FileStore {
  ...
  listArticles(options: { includeUnapproved?: boolean; authorId?: string } = {}) {
    ...
    return sortByNewest(
      this.requireData().articles.filter((article) => {
        ...
        if (authorId) {
          const published = ['APPROVED', 'PUBLISHED'].includes(article.status ?? '');
          if (!published && article.submittedBy !== authorId) return false;
        }
        return true;
      })
    );
  ...
  listMagazines(options: { includeUnapproved?: boolean; authorId?: string } = {}) {
    ...
    return sortByNewest(
      this.requireData().magazines.filter((magazine) => {
        ...
        if (authorId) {
          const published = ['APPROVED', 'PUBLISHED'].includes(magazine.status ?? '');
          if (!published && magazine.submittedBy !== authorId) return false;
        }
        return true;
      })
    );
  ...
  async updateSubscriptionRequest(id: string, status: SubscriptionStatus, reviewer?: UserRecord, rejectionReason?: string) {
    ...
    data.subscriptionRequests[index] = {
      ...
      rejectionReason: status === 'REJECTED' ? rejectionReason ?? data.subscriptionRequests[index].rejectionReason : undefined,
      updatedAt: nowIso()
    };
    await this.persist();
