...
async function start() {
  await Promise.all(UPLOADS_DIR_CANDIDATES.map((dir) => fs.mkdir(dir, { recursive: true })));
  await store.init();
  ...
  // Static files
  for (const dir of UPLOADS_DIR_CANDIDATES) {
    app.use('/uploads', express.static(dir));
    app.use('/api/uploads', express.static(dir));
  }
  // Compatibility alias for legacy asset URLs using /api/uploads/*
  ...
  app.listen(PORT, () => {
    ...
    console.log(`Uploads directories: ${UPLOADS_DIR_CANDIDATES.join(' | ')}`);
  });
}
