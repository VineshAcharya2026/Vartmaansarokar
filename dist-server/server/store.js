...
function withTimestamps(value, createdAt = nowIso()) {
    return {
        ...value,
        createdAt,
        updatedAt: createdAt
...
async function createDefaultDatabase() {
    ...
    const users = await Promise.all(DEFAULT_ACCOUNTS.map(async (account, index) => withTimestamps({
        id: createId('user'),
        email: account.email,
        name: account.name,
        role: account.role,
...
export class FileStore {
    ...
    listArticles(options = {}) {
        ...
        return sortByNewest(this.requireData().articles.filter((article) => {
            if (!includeUnapproved && !['APPROVED', 'PUBLISHED'].includes(article.status ?? ''))
                return false;
            if (authorId && article.submittedBy !== authorId)
                return false;
            return true;
        }));
    }
    ...
    async reviewArticle(id, status, input) {
    ...
    listMagazines(options = {}) {
        const { includeUnapproved = true, authorId } = options;
        return sortByNewest(this.requireData().magazines.filter((magazine) => {
            if (!includeUnapproved && !['APPROVED', 'PUBLISHED'].includes(magazine.status ?? ''))
                return false;
            if (authorId && magazine.submittedBy !== authorId)
                return false;
            return true;
        }));
    }
    ...
    async createMagazine(input) {
        const magazine = withTimestamps({
            ...input,
    ...
    async reviewMagazine(id, status, input) {
    ...
    async replaceAds(ads) {
        this.requireData().ads = ads.map((ad, index) => withTimestamps({ ...ad, id: ad.id || createId('ad') }, new Date(Date.now() - index * 1000).toISOString()));
        await this.persist();
        return this.listAds();
    }
    listMedia(filters = {}) {
        ...
        return sortByNewest(this.requireData().media.filter((item) => {
    ...
    async createSubscriptionRequest(input) {
    ...
    async ensureDefaultAccounts() {
        ...
        for (const [index, account] of DEFAULT_ACCOUNTS.entries()) {
            ...
            if (!existing) {
                data.users.push(withTimestamps({
                    id: createId('user'),
                    email: account.email,
                    name: account.name,
                    role: account.role,
