...
const Category: React.FC = () => {
  ...
  const filteredNews = news.filter(n => n.category.toLowerCase() === categoryName?.toLowerCase());
    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-gray-100 pb-8 gap-4">
        <div>
          <nav className="flex items-center text-xs font-bold text-gray-400 space-x-2 mb-2 uppercase tracking-widest">
            ...
            <span>/</span>
            <span className="text-[#800000]">Category</span>
          </nav>
          <h1 className="text-4xl md:text-5xl font-bold text-[#001f3f] serif">{categoryName}</h1>
        </div>
        <p className="text-gray-500 max-w-md text-sm">
          Showing {filteredNews.length} articles in the {categoryName} section. Stay updated with the latest in this field.
        </p>
      </div>
      ...
      {filteredNews.length > 0 ? (
              ...
              <Link to={`/news/${item.id}`}>
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                    ...
                    <span className="text-xs font-bold text-[#001f3f]">By {item.author}</span>
                  </div>
                </div>
              </Link>
            </article>
        ...
        <div className="text-center py-20 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
          <h2 className="text-2xl font-bold text-gray-400 serif">No articles found in this category</h2>
          <Link to="/" className="text-[#800000] font-bold mt-4 inline-block hover:underline">Explore other sections</Link>
        </div>
      )}
    </div>
  );
