...
import { BookOpen, Star, ShieldCheck, Mail, Smartphone } from 'lucide-react';
...
const Magazine: React.FC = () => {
  ...
  const magazineList = Array.isArray(magazines) ? magazines : [];

  const selectedMag = magazineList.find((item) => item.id === selectedMagId) ?? magazineList[0] ?? null;

  useEffect(() => {
    if (!selectedMagId && magazineList[0]) {
      setSelectedMagId(magazineList[0].id);
      return;
    }

    if (selectedMagId && !magazineList.some((item) => item.id === selectedMagId)) {
      setSelectedMagId(magazineList[0]?.id ?? null);
    }
  }, [magazineList, selectedMagId]);
  ...
  return (
    <div ref={pageRef} className="space-y-16 py-10">
      {/* HEADER SECTION */}
      <div data-magazine-reveal className="max-w-6xl mx-auto bg-gradient-to-br from-[#001f3f] to-[#0f172a] rounded-[36px] px-8 md:px-14 py-12 md:py-14 text-white shadow-2xl border border-white/10">
        <div className="flex flex-col lg:flex-row gap-10 lg:items-center lg:justify-between">
          <div className="max-w-2xl space-y-4">
            <span className="inline-flex items-center gap-2 bg-white/10 text-white text-[10px] font-black uppercase tracking-[0.3em] px-4 py-2 rounded-full">
              Digital Reading Room
            </span>
            <h1 className="text-4xl md:text-6xl font-black serif tracking-tight leading-tight">
              Monthly <span className="text-[#fca5a5]">Library</span>
            </h1>
            <p className="text-blue-100/80 text-base md:text-lg font-medium max-w-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="bg-white/10 rounded-2xl px-5 py-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-blue-100/70">Total Issues</p>
              <p className="text-2xl font-black">{magazineList.length}</p>
            </div>
            <div className="bg-white/10 rounded-2xl px-5 py-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-blue-100/70">Selected Issue</p>
              <p className="text-sm font-black truncate">{selectedMag.issueNumber}</p>
            </div>
            <div className="bg-white/10 rounded-2xl px-5 py-4">
              <p className="text-[10px] font-black uppercase tracking-widest text-blue-100/70">Access</p>
              <p className="text-sm font-black">{selectedMag.isFree ? 'Free' : 'Subscriber'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <section data-magazine-reveal className="bg-white p-6 md:p-12 rounded-[32px] shadow-2xl border border-gray-100">
        ...
        <div className="mb-10 grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3">
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Publish Date</p>
            <p className="text-sm font-bold text-[#001f3f] mt-1">{selectedMag.date}</p>
          </div>
          <div className="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3">
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Issue Type</p>
            <p className="text-sm font-bold text-[#001f3f] mt-1">{selectedMag.isFree ? 'Public Preview' : 'Premium Issue'}</p>
          </div>
          <div className="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3">
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Paywall Starts</p>
            <p className="text-sm font-bold text-[#001f3f] mt-1">Page {selectedMag.gatedPage ?? 2}</p>
          </div>
        </div>
