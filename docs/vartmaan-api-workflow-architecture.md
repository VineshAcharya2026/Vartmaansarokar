...
│
├─ 💻 Frontend Code
│  ├─ vartmaan-frontend-index.tsx → React entry point
│  ├─ vartmaan-frontend-app.tsx → Main component
│  ├─ vartmaan-frontend-app-context.tsx → Global state
...
│
├─ 📊 Database
│  ├─ db/vartmaan-db-schema.sql → Database structure
│  ├─ db/seeds/seed.sql → Sample data
│  └─ db/migrations/*.sql → Schema updates
│
└─ 🌍 i18n & Assets
   ├─ locales/ → 13 language translations
   ├─ public/ → Static assets
   └─ vartmaan-frontend-styles.css → Global styles
```
